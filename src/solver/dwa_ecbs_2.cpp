...
#include "dwa_ecbs_2.h"
#include "../util/util.h"
#include<bits/stdc++.h>
#include <boost/algorithm/string.hpp>
using namespace std;

uint64_t DWA_ECBS_2::timeSinceEpochMillisec() {
  using namespace std::chrono;
  return duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
}
DWA_ECBS_2::DWA_ECBS_2(Problem* _P, float _w) : CBS(_P, false), w(_w) {
  init();
}
DWA_ECBS_2::DWA_ECBS_2(Problem* _P, float _w, bool _ID) : CBS(_P, _ID), w(_w) {
  init();
}

DWA_ECBS_2::~DWA_ECBS_2() {}

void DWA_ECBS_2::init() {
...
bool DWA_ECBS_2::solvePart(Paths& paths, Agents& block) {
  //int highLevelNode = 0;
  CTNode* node;
...
void DWA_ECBS_2::invoke(CTNode* node, Agents& block) {
  int d;
  cout << "invoke" << endl;
  // calc path
...
int DWA_ECBS_2::bin_search(std::vector<conflicted_node*> & conflicted_list, int st, int en, int time){
...
int DWA_ECBS_2::h3(Agent* a, Nodes &p1, Paths &paths) {
...
void DWA_ECBS_2::print_paths(Paths &paths){
  for(int i=0; i<paths.size();i++){
    Nodes p2 = paths[i];
...
Nodes DWA_ECBS_2::AstarSearch(Agent* a, CTNode* node) {
...
void DWA_ECBS_2::getPartialPath(AN* n, Nodes &path) {
  path.clear();
  AN* m = n;
  while (m != nullptr) {
    path.push_back(m->v);
