...
uint64_t SWA_ECBS::timeSinceEpochMillisec() {
  using namespace std::chrono;
  return duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
}

SWA_ECBS::SWA_ECBS(Problem* _P, float _w) : CBS(_P, false), w(_w) {
  init();
}
SWA_ECBS::SWA_ECBS(Problem* _P, float _w, bool _ID) : CBS(_P, _ID), w(_w) {
  init();
}
...
void SWA_ECBS::getPartialPath(AN* n, Nodes &path) {
  path.clear();
  AN* m = n;
  while (m != nullptr) {
    path.push_back(m->v);
